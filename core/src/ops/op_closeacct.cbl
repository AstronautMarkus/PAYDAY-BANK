*> CLOSEACCT <customer-id> <account>
*> Terminally closes an account owned by the customer. Requires the
*> balance to be exactly zero first (withdraw or transfer it out
*> before closing). CLOSED is terminal -- there is no reopen operation.
IDENTIFICATION DIVISION.
PROGRAM-ID. OP-CLOSEACCT.
        ...
        ELSE
            ...
            ELSE
                IF WS-ACCOUNT-STATUS = "CLOSED"
                    DISPLAY "ERR|Account is already closed"
                ELSE
                    IF WS-BALANCE NOT = 0
                        DISPLAY "ERR|Account balance must be zero to close"
                    ELSE
                        MOVE "CLOSED" TO WS-ACCOUNT-STATUS
                        MOVE "REWRITE" TO WS-REPO-FUNCTION
                        CALL "ACCOUNT-REPOSITORY" USING BY REFERENCE WS-REPO-FUNCTION
                            WS-ACCOUNT-RECORD WS-REPO-FOUND WS-REPO-EOF
                        DISPLAY "OK|Account closed"
                    END-IF
                END-IF
            END-IF
        END-IF
