*> BLOCKCARD <customer-id> <card-number>
*> Blocks a card owned by the customer; CARDPAYMENT refuses any card
*> that is not ACTIVE. Reversed by UNBLOCKCARD.
IDENTIFICATION DIVISION.
PROGRAM-ID. OP-BLOCKCARD.
...
MAIN-OP-BLOCKCARD.
    ...
    ELSE
        ...
        ELSE
            ...
            ELSE
                IF WS-CARD-STATUS = "BLOCKED"
                    DISPLAY "ERR|Card is already blocked"
                ELSE
                    MOVE "BLOCKED" TO WS-CARD-STATUS
                    MOVE "REWRITE" TO WS-CARD-REPO-FUNCTION
                    CALL "CARD-REPOSITORY" USING BY REFERENCE WS-CARD-REPO-FUNCTION
                        WS-CARD-RECORD WS-CARD-REPO-FOUND WS-CARD-REPO-EOF
                    DISPLAY "OK|Card blocked"
                END-IF
            END-IF
        END-IF
    END-IF
